...
int drv8846_parse_dt(struct drv8846_soc_ctrl *mctrl)
{
	...
	if (!gpio_is_valid(mctrl->gpio_mode0)) {
		pr_err("mctrl->motor_data.gpio_mode0 is invalid.");
		return -EINVAL;
	}
	...
	if (!gpio_is_valid(mctrl->gpio_mode1)) {
		pr_err("motor,mode1-gpio is invalid.");
		return -EINVAL;
	}
	...
	if (!gpio_is_valid(mctrl->gpio_sleep)) {
		pr_err("motor,sleep-gpio is invalid.");
		return -EINVAL;
	}
	...
	if (!gpio_is_valid(mctrl->gpio_dir)) {
		pr_err("motor,dir-gpio is invalid.");
		return -EINVAL;
	}
	...
	if (!gpio_is_valid(mctrl->gpio_pwren)) {
		pr_err("power,en-gpio is invalid.");
		return -EINVAL;
	}
	...
	if (rc < 0) {
		pr_err("motor,rampup-pwm-period-ns not set, use default.");
	...
	if (rc < 0) {
		pr_err("motor,high-pwm-period-ns not set, use default.");
	...
	if (rc < 0) {
		pr_err("motor,rampdown-pwm-period-ns not set, use default.");
	...
	if (rc < 0) {
		pr_err("motor,rampup-duration-ms not set, use default.");
	...
	if (rc < 0) {
		pr_err("motor,high-duration-ms not set, use default.");
	...
	if (rc < 0) {
		pr_err("motor,rampdown-duration-ms not set, use default.");
	...
	rc = of_property_read_u32(of_node, "motor,step-mode",
				  &mctrl->step_mode);
	if (rc < 0) {
		pr_err("motor,step-mode not set, use default.");
