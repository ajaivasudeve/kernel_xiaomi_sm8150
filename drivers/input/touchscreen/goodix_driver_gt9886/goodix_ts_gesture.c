...
int goodix_sync_ic_stat(struct goodix_ts_core *core_data)
{
	...
	if (tp_stat == TP_GESTURE_DBCLK) {
		ts_debug("sync IC suspend stat from DBCLK to DBCLK_FOD");
		...
		ts_debug("sync IC suspend stat from SLEEP to FOD");
...
static int goodix_set_suspend_func(struct goodix_ts_core *core_data)
{
	...
	if (core_data->double_wakeup && core_data->fod_status) {
		...
		ts_debug("Set IC double wakeup mode on,FOD mode on;");
	} else if (core_data->double_wakeup && (!core_data->fod_status)) {
		state_data[0] = GSX_GESTURE_CMD;
		state_data[1] = 0x03;
		state_data[2] = 0xF5;
		ret = goodix_i2c_write(dev, GSX_REG_GESTURE, state_data, 3);
		ts_debug("Set IC double wakeup mode on,FOD mode off;");
	} else if (!core_data->double_wakeup && core_data->fod_status) {
		state_data[0] = GSX_GESTURE_CMD;
		state_data[1] = 0x00;
		state_data[2] = 0xF8;
		ret = goodix_i2c_write(dev, GSX_REG_GESTURE, state_data, 3);
		ts_debug("Set IC double wakeup mode off,FOD mode on;");
	} else if (!core_data->double_wakeup && (!core_data->fod_status)) {
		state_data[0] = GSX_GESTURE_CMD;
		state_data[1] = 0x02;
		state_data[2] = 0xF6;
		ret = goodix_i2c_write(dev, GSX_REG_GESTURE, state_data, 3);
		ts_debug("Set IC double wakeup mode off,FOD mode off;");
	} else {
		ret = -1;
		ts_debug("Get IC mode falied,core_data->double_wakeup=%d,core_data->fod_status=%d;",
