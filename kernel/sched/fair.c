...
cpu_is_in_target_set(struct task_struct *p, int cpu)
{
	...
	int first_cpu = (schedtune_prefer_high_cap(p, true)) ? rd->mid_cap_orig_cpu :
							 rd->min_cap_orig_cpu;
...
static bool is_packing_eligible(struct task_struct *p, int target_cpu,
				...
				int best_idle_cstate, bool prefer_high_cap)
{
				   ...
				   bool prefer_high_cap)
{
	...
	*backup_cpu = -1;
	...
	if (target_cpu != -1 && !idle_cpu(target_cpu) &&
		...
		if (curr_tsk && schedtune_prefer_high_cap(curr_tsk, false)) {
			target_cpu = best_idle_cpu;
		}
	}
				     ...
				     int sync, bool sync_boost)
{
	...
	bool prefer_high_cap = schedtune_prefer_high_cap(p, true);
	fbt_env.fastpath = 0;
